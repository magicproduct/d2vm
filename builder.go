...
func (b *builder) chWriteFileIfNotExist(path string, content string, perm os.FileMode) error {
	...
	if err == nil {
		if (i.Mode() & os.ModeSymlink) != 0 {
			// On some OSes (and containers) /etc/resolv.conf is symlink
			// (to a file generated by systemd-resolve)
			logrus.Warnf("path %s is unexpectedly a symlink, ignoring", path)
			return nil
		} else if (i.Mode() & os.ModeDir) != 0 {
			logrus.Warnf("path %s is unexpectedly a symlink, ignoring", path)
			return nil
		} else if i.Size() > 0 {
			return nil
